...
The cost is concrete in the developer journey I walked ([How I audited](#how-i-audited)). To get an API key you must enter **Studio**, which presents itself as a standalone product with its own getting-started rather than the console tool it is. The guidance for *building* is then split three ways: the API surface itself (conversations, agents, RAG) is filed under **Studio**, more developer guidance sits in the **Developers** section, and the quickstarts sit under **Getting Started**. Three homes, and the reader assembles the path. The [platform overview](https://docs.mistral.ai/getting-started/platform-overview) compounds it by opening on a product picker ("three products, pick one") instead of the build journey the developer came for. And it starts one level up: the docs home (`docs.mistral.ai`) itself leads with three product cards (Vibe, Studio, Admin), the same product-first framing, which the [Appendix landing-page design](/appendix#2-landing-page-design) replaces with persona entry points. The same confusion repeats one level down: quickstarts group by persona (developer, admin, vibe-work) even though each quickstart is already a task, and persona and product segments in URLs churn on every rename, as the retired `le-chat` paths show.
