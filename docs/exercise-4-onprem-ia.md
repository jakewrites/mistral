...
- **A deprecation inside a version propagates to every surface.** The audit caught this failing live: fine-tuning carries a deprecation banner while its cookbooks still teach it with no notice ([the freshness note](/exercise-1/audit-findings#a-freshness-issue-validation-surfaced)). The rule per version is the same as the rule site-wide. A deprecation is not done until every page, example, and cookbook that teaches the capability is banner-marked or retired.
