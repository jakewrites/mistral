...
This exercise dovetails with Exercise 1, and the two meet on a clean split. [R2](/exercise-1/recommendations#r2) treats managed and self-hosted as a **surface** for the content both deployments share. The task-based user guides (call the API, build a RAG pipeline, write a prompt) read almost identically whether Mistral runs the service or you do, so those stay one set of pages with a deployment-mode and version selector, not two copies.
...
- **From search.** The surface marker ([R2](/exercise-1/recommendations#r2)) keeps a deep landing oriented to its surface.
...
Managed docs need none of this. The product has one live version, so the docs are continuously deployed and unversioned. Self-hosted docs cannot work that way: an operator runs the version they installed, sometimes for years, and documentation for a version they do not run describes someone else's cluster. [R2](/exercise-1/recommendations#r2) gates self-hosted content by version. This section is the policy behind the gate.
...
- **Every supported version is hosted and switchable.** Self-hosted pages carry a version selector, the way Kubernetes serves its current and recent releases and Terraform Enterprise versions its self-hosted operator docs by release. The latest supported version lives at the unprefixed, canonical URL. Older supported versions live under a version segment (`/vibe/self-hosted/v2.5/...`), and the reader's choice is remembered across pages.
...
| Versioned docs with an archive policy | Kubernetes and Terraform Enterprise versioned docs | Operators run old versions for years, so the docs must match what they run |

## Where the models fail, and what this design does instead

Borrowing only the good parts would be uncritical. Three specific failures in these docs shaped this design as much as their strengths did:
...
- **Kubernetes threads version conditions through pages that otherwise read as version-neutral.** A task page shows the latest content and leaves the reader to catch a feature-state badge and a single prerequisite line ([*"your Kubernetes server must be at or later than version 1.34"*](https://kubernetes.io/docs/tasks/configure-pod-container/assign-pod-level-resources/), plus a feature gate to switch on) and self-filter for the version they actually run. On a long page that one line is easy to skim past, and an operator on an older cluster can work halfway through a procedure their install does not support. Serving version snapshots is the right half of Kubernetes' model, and this IA borrows it; leaving the conditionals loose inside the page is the half to fix. Here the tree is gated per version ([Versioning](#versioning)), so a page describes the reader's version rather than the newest one, and any requirement that survives (a feature added in a later patch, a flag to enable) is a surface marker at the top of the page, not a line in a prerequisites list.
- **Terraform makes you know its product names before you can navigate.** Once you reach them, [Terraform Enterprise's self-hosted docs](https://developer.hashicorp.com/terraform/enterprise/deploy) are well-structured: the operator lifecycle is organised around goals (Deploy, migrate, administer, manage access), each a task rather than a tool, and versioned by release. That structure is close to the one this IA proposes, and its per-release versioning is the model behind the [version gate](#versioning). The failure sits upstream of it, at the entry points. You have to already know the self-hosted product is called *Terraform Enterprise*, as opposed to HCP Terraform or the CLI, to find its docs at all, and there is no goal-based door that reads *run this yourself*. That is [Exercise 1's core finding](/exercise-1/executive-summary#the-core-finding) in another vendor's docs: the navigation names the product where it should name the job. This IA fixes it at the door, with the three goal-based entry points in [How readers reach this](#entry), so *"I want to run Mistral on my own infrastructure"* is a way in, not a product name the reader must already know.
